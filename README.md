...
```bash
git clone https://github.com/mdhmrunal31/Practice.git
cd Practice
```

### 2. Create and activate a virtual environment

> ⚠️ Use **one dedicated venv per project**. Activating a venv from a different project folder in the same terminal session is the most common source of `ModuleNotFoundError` — always confirm with `where python` / `pip show <package>` that you're pointed at *this* project's `.venv`.
...
Keep this terminal open and running for the whole session.

```bash
python -m uvicorn backend.main:app --reload --port 8000
```
Backend runs at `http://localhost:8000` — verify it's healthy:
```bash
...
In a **separate** terminal window (don't stop the backend to run this):
```bash
cd frontend
npm install
npm run dev
```
Frontend runs at `http://localhost:5173` and opens automatically in your browser.

Enter any research topic and click **Run pipeline**.

> Both terminals must stay running at the same time — one serves the API on port 8000, the other serves the UI on port 5173.
...
- `.venv/` and `frontend/node_modules/` are listed in `.gitignore` — never pushed to GitHub
